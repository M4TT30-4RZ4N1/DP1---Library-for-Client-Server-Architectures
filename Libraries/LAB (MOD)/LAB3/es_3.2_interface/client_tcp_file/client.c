...
int main (int argc, char *argv[]) {
	...
	err_getaddrinfo=getaddrinfo(dest_h, dest_p, NULL, &list);
	if(err_getaddrinfo==-1){err_quit("(%s) error on getaddrinfo",prog_name);}
	...
	err_msg("(%s) connected to %s:%u", prog_name, inet_ntoa(destaddr.sin_addr), ntohs(destaddr.sin_port));
...
/*SERVER*/	if (FD_ISSET(sockfd, &rset)) {
			...
			if (flag_reading_file == 0) { //NO READING FILE
				...
				if (nread >= strlen(MSG_OK) && strncmp(buf,MSG_OK,strlen(MSG_OK))==0) {
					...
					if(n<=0){ err_msg("(%s) --- error file_size read",prog_name);}
					file_bytes = ntohl((*(uint32_t *)buf));
					err_msg("(%s) --- received file size '%u'",prog_name, file_bytes);
					...
					int time;		
					n= Read(sockfd, buf, 4);
					if(n<=0){ err_msg("(%s) --- error file_time read",prog_name);}
					...
					err_msg("(%s) --- received time information '%s'",prog_name, buf);
                        		
					//file open
					if ( (fp=fopen(fnamestr, "w+"))!=NULL) {
						err_msg("(%s) --- opened file '%s' for writing",prog_name, fnamestr);
						file_ptr = 0;   //COUNTER THAT AT THE END OF TRANSFER MUST BE EQUAL TO SIZE OF THE FILE
						flag_reading_file = 1; //I' M READING!!!!
					} else {
						err_quit("(%s) --- cannot open file '%s'",prog_name, fnamestr);
					}
				} else if (nread >= strlen(MSG_ERR) && strncmp(buf,MSG_ERR,strlen(MSG_ERR))==0) {
					err_msg("(%s) - received '%s' from server: maybe a wrong request?", prog_name, buf);
				} else {
					err_msg("(%s) - protocol error: received response '%s'", prog_name, buf);
					break;
				}
				...
				if (file_ptr == file_bytes) {
					fclose(fp);
					err_msg("(%s) --- received and wrote file '%s'",prog_name, fnamestr);
					flag_reading_file = 0;
				}

			} else {
				err_msg("(%s) - flag_reading_file error '%d'", prog_name, flag_reading_file);
				break;
			}
		}
/*KEYBOARD*/    if (FD_ISSET(fileno(stdin), &rset)) {
			...
			if ( (fgets(command, MAXBUFL, stdin)) == NULL) { 
				...
				err_msg("(%s) --- GET command sent to server",prog_name);
