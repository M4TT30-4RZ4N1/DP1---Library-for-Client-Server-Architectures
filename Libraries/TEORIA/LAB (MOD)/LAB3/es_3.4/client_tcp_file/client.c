...
int main (int argc, char *argv[]) {
	...
	if ((n=select(FD_SETSIZE, &mysockets, NULL, NULL, &tv))==-1) {
		...
		if (nread >= strlen(MSG_OK) && strncmp(buf,MSG_OK,strlen(MSG_OK))==0) {
			...
			n = read(sockfd, buf, 4);
			struct tm *time;
			time = localtime(&buf);//conversion time stat->local time
			
			
			trace( err_msg("(%s) --- received file at time: '%u'",prog_name, asctime(time) ) );
...
else{ printf("ATTENTION !!! INVALID COMMAND !!! ERROR !!! \n");}
