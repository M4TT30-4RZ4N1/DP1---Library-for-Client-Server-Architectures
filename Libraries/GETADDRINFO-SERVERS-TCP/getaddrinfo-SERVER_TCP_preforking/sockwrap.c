...
/*Functions created by s236462 Matteo Arzani - Politecnico Di Torino */


// in main() before call this: do->  
// struct sockaddr_in6 serveraddr; |memset(&serveraddr, 0, sizeof(serveraddr));| sd= init_Server(TCP, &serveraddr);
// then i must listen and connect
...
int init_Server_dual_stack(char* protocol,int port,struct sockaddr_in6* pointer){
...
//SERVER IPV4/IPV6 NOT DUAL STACK
//serv = porta
	...
	int sfd, s; 
	 ...
	 memset(&hints, 0, sizeof(struct addrinfo));
         ...
         hints.ai_next = NULL;
           ...
           s = getaddrinfo(NULL, serv, &hints, &result);
           if (s != 0) {
...
//pointer is: &clientaddr, nb sdconn is a socket after the connect
void print_client_ipv6_address_tcp(int accepted_socket,char* progname, struct sockaddr_in6* pointer, socklen_t* size){

	 char str[INET6_ADDRSTRLEN];
	   
	 getpeername(accepted_socket, (struct sockaddr *) pointer, size);
         if(inet_ntop(AF_INET6, &pointer->sin6_addr, str, sizeof(str))) {
            printf("(%s) - Client address is %s\n",progname, str);
            printf("(%s) - Client port is %d\n",progname, ntohs(pointer->sin6_port));
         }
}

void print_client_ipv4_address_tcp(int accepted_socket,char* progname, struct sockaddr_in* pointer, socklen_t* size){

	 char str[16];
	   
	 getpeername(accepted_socket, (struct sockaddr *) pointer, size);
         if(inet_ntop(AF_INET, &pointer->sin_addr, str, sizeof(str))) {
            printf("(%s) - Client address is %s\n",progname, str);
            printf("(%s) - Client port is %d\n",progname, ntohs(pointer->sin_port));
         }
}

void retrieve_client_ip6_port(int accepted_socket,char* progname, struct sockaddr_in6* pointer, socklen_t* size,char* ip, char* port){
	 ...
	 getpeername(accepted_socket, (struct sockaddr *) pointer, size);
         if(inet_ntop(AF_INET6, &pointer->sin6_addr, str, sizeof(str))) {
            sprintf(ip,"%s",str);
            sprintf(port,"%d",ntohs(pointer->sin6_port));
         }
}

void retrieve_client_ip4_port(int accepted_socket,char* progname, struct sockaddr_in* pointer, socklen_t* size,char* ip, char* port){
	 ...
	 getpeername(accepted_socket, (struct sockaddr *) pointer, size);
         if(inet_ntop(AF_INET, &pointer->sin_addr, str, sizeof(str))) {
            sprintf(ip,"%s",str);
            sprintf(port,"%d",ntohs(pointer->sin_port));
         }
}


void from_ip6_to_ip4(char* ip){
...
//read from the keyboard a command and insert it into the buffer
void read_keyboard(char* buf,char* layout,char* progname){

	int n=0;
	char c;
	
	printf("(%s) - Insert command %s:\n",progname,layout);
...
void add_end_specials(char* buf){
...
//remove \r \n at the end of buffer
void remove_end_specials(char* buf){
...
//read from the tcp socket a message
int read_socket_message(int mysocket,char* buf){
